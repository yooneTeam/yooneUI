...
export default function RssSetting({ id }) {
    ...
    const fetcher = url => url
        ...
        axios.get(urlAPI, { params: { url } })
            .then(res => { setRssData(res.data) })
            .catch(err => { setInfoText('無効なURLです') })
        ...
        <>
            {rssUrlList.map(rss =>
                <div key={rss.url} >
                    <ListItem
                        secondaryAction={
                            <IconButton edge="end" size='small' onClick={() => { onClickRemove(rss.url) }}>
            ...
            <ListItem>
                <Stack
                    ...
                    justifyContent='space-between' sx={{ width: '100%', mt: '-8px' }}
                >
                    <Input
                        onChange={onChange}
                        size="small"
                    ...
                    <Button
                        ...
                        disabled={Boolean(
                            !rssData[0]
                            ||
                            rssUrlList.map(rss => rss.url).includes(rssData[0].rssURL)
                        )}
                        sx={{ ml: '12px' }}
                    >
                        追加
