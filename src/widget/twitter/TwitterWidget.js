...
export default function Twitter({ id }) {
  const fetcher = () =>
    axios
      .get(
        'https://asia-northeast1-yooneapi.cloudfunctions.net/twitterGetTweet?userid=959072014899097601',
      )
      .then((res) => res.data)
          ...
          <div key={tweet.text}>
            <ListItem disablePadding sx={{ my: '10px', px: '4%' }}>
              <Stack sx={{ mb: '4px', width: '100%' }}>
                ...
                <Typography fontSize='0.85rem'>{tweet.referencedText || tweet.text}</Typography>
